...
export function printReport(
  ...
  const tableRows = rows
    .map(
      (row) =>
        `<tr>${row.map((c) => `<td style="padding:6px 10px;border-bottom:1px solid #eee;font-size:12px">${c}</td>`).join('')}</tr>`,
    )
    .join('')

  const totalsRow = totals
    ? `<tr style="border-top:2px solid #333;font-weight:800">${totals.map((c) => `<td style="padding:8px 10px;font-size:12px">${c}</td>`).join('')}</tr>`
    : ''

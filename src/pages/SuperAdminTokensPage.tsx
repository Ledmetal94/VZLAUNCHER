...
import {
  getSuperAdminTokenTransactions,
  getSuperAdminVenues,
  type SuperAdminTokenTransaction,
  type CloudVenue,
...
function parseGameFromRef(ref: string | null): string {
  ...
  return match ? match[1] : ref.slice(0, 24)
}

export default function SuperAdminTokensPage() {
  const navigate = useNavigate()
  const [transactions, setTransactions] = useState<SuperAdminTokenTransaction[]>([])
  ...
  useEffect(() => {
    getSuperAdminVenues().then(setVenues).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    const filters: { venueId?: string; type?: string; startDate?: string; endDate?: string } = {}
    ...
    getSuperAdminTokenTransactions(page, pageSize, filters)
      .then((res) => {
        if (active) {
    ...
    return () => { active = false }
  }, [page, rangeDays, typeFilter, venueFilter])
      ...
      <div className="relative z-10 flex flex-1 flex-col min-h-0 p-8">
        ...
        <div className="flex items-center justify-between" style={{ marginBottom: 24 }}>
          <div className="flex items-center gap-4">
            ...
            <h1 style={{ fontSize: 22, fontWeight: 800 }}>Transazioni Gettoni</h1>
            ...
            {transactions.length > 0 && (
              <button
                onClick={() => {
                  const headers = ['Data', 'Sede', 'Tipo', 'Importo', 'Metodo', 'Riferimento', 'Stato']
                  const rows = transactions.map((tx) => [
                    formatDate(tx.created_at), tx.venueName,
                    TYPE_LABELS[tx.type]?.label || tx.type, String(tx.amount),
                    tx.payment_method ? (METHOD_LABELS[tx.payment_method] || tx.payment_method) : '',
                    tx.type === 'consume' ? parseGameFromRef(tx.payment_reference) : (tx.payment_reference || ''),
                    tx.status,
                  ])
                  downloadCsv('transazioni-gettoni', headers, rows)
                }}
                style={{ padding: '5px 12px', borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: 'pointer', border: '1px solid rgba(123,100,169,0.15)', background: 'rgba(255,255,255,0.03)', color: 'rgba(255,255,255,0.5)', fontFamily: 'Outfit, sans-serif' }}
              >
                CSV
              </button>
            )}
        ...
        <div className="flex items-center gap-2" style={{ marginBottom: 16 }}>
          {RANGE_OPTIONS.map((r) => (
            <button key={r.days} style={chipStyle(rangeDays === r.days)} onClick={() => { setRangeDays(r.days); setPage(1) }}>
              {r.label}
            </button>
          ))}
          ...
          <select value={venueFilter} onChange={(e) => { setVenueFilter(e.target.value); setPage(1) }} style={selectStyle}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(123,100,169,0.1)' }}>
                {['Data', 'Sede', 'Tipo', 'Importo', 'Metodo', 'Riferimento', 'Stato'].map((h) => (
                  <th
                    key={h}
                    style={{
            ...
            <tbody>
              {loading ? (
                Array.from({ length: 8 }).map((_, i) => (
                  <tr key={i}><td colSpan={7} style={{ padding: '8px 16px' }}><SkeletonRow /></td></tr>
                ))
              ) : transactions.length === 0 ? (
                <tr>
                  <td colSpan={7} style={{ padding: 40, textAlign: 'center', color: 'rgba(255,255,255,0.25)' }}>
                ...
                transactions.map((tx) => {
                  ...
                  return (
                    <tr key={tx.id} style={{ borderBottom: '1px solid rgba(123,100,169,0.06)' }}>
                      <td style={{ padding: '10px 16px', color: 'rgba(255,255,255,0.5)' }}>{formatDate(tx.created_at)}</td>
                      <td style={{ padding: '10px 16px', fontWeight: 600, fontSize: 12 }}>{tx.venueName}</td>
                      ...
                      <td style={{
                        padding: '10px 16px', fontWeight: 700, fontVariantNumeric: 'tabular-nums',
                      ...
                      <td style={{ padding: '10px 16px' }}>
                        <span style={{
                          ...
                          color: tx.status === 'confirmed' ? '#44ff88' : tx.status === 'failed' ? '#ff4444' : '#f59e0b',
                        }}>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2" style={{ marginTop: 16 }}>
            <button disabled={page <= 1} onClick={() => setPage(page - 1)}
              style={{ ...chipStyle(false), opacity: page <= 1 ? 0.3 : 1, cursor: page <= 1 ? 'default' : 'pointer' }}>
              ← Prec
            </button>
            ...
            <button disabled={page >= totalPages} onClick={() => setPage(page + 1)}
              style={{ ...chipStyle(false), opacity: page >= totalPages ? 0.3 : 1, cursor: page >= totalPages ? 'default' : 'pointer' }}>
              Succ →
            </button>
          </div>
        )}
